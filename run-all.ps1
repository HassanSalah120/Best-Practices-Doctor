...
if (-not [string]::IsNullOrWhiteSpace($McpWorkspaceRoot)) {
    $effectiveMcpWorkspaceRoot = [System.IO.Path]::GetFullPath($McpWorkspaceRoot)
} elseif (-not [string]::IsNullOrWhiteSpace($env:BPDOCTOR_WORKSPACE_ROOT)) {
    $effectiveMcpWorkspaceRoot = [System.IO.Path]::GetFullPath($env:BPDOCTOR_WORKSPACE_ROOT)
}
$npmCmd = "npm.cmd"
try {
...
$mcpDir = Join-Path $repoRoot "bpdoctor-mcp"

if (-not $SkipInstall) {
    $setupScript = Join-Path $repoRoot "setup.ps1"
    if (Test-Path $setupScript) {
        Write-Status "Running quick setup check..." "INFO"
...
if (Test-Path $venvPython) {
    $env:VIRTUAL_ENV = Join-Path $repoRoot "backend\.venv"
    $env:PATH = "$venvScripts;$env:PATH"
    Write-Status "Using backend virtualenv: backend\.venv" "SUCCESS"
}
...
Ensure-NpmInstall (Join-Path $repoRoot "tauri")
Ensure-NpmInstall $mcpDir
    ...
    "mcpServers" = @{
        "bpdoctor" = @{
            ...
            "env" = @{
                ...
                "BPDOCTOR_WORKSPACE_ROOT" = $effectiveMcpWorkspaceRoot
            }
        }
    }
}
...
$env:BPDOCTOR_WORKSPACE_ROOT = $effectiveMcpWorkspaceRoot

$mcpProc = Start-Process -FilePath $npmCmd -ArgumentList @("run", "dev") -WorkingDirectory $mcpDir -PassThru -NoNewWindow -RedirectStandardOutput $mcpOut -RedirectStandardError $mcpErr
...
Write-Host "  MCP Workspace Root: $effectiveMcpWorkspaceRoot"
Write-Host "  MCP Config: $configPath"
Write-Host "  Portable MCP Config: $portableConfigPath"
Write-Host "`nLive Logs:"
Write-Host "  Get-Content -Wait $tauriOut"
Write-Host "  Get-Content -Wait $tauriErr"
Write-Host "  Get-Content -Wait $mcpOut"
Write-Host "  Get-Content -Wait $mcpErr"
Write-Host "`nIDE MCP Configuration:"
Write-Host "  Copy the contents of: $configPath"
Write-Host "  If your IDE rejects cwd/workingDirectory, copy: $portableConfigPath"
Write-Host "  Or use these settings in your IDE:"
Write-Host "  - Command: $npmCmd"
Write-Host "  - Args: run dev"
Write-Host "  - Working Directory: $mcpDir"
