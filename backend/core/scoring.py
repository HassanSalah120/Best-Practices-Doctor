...
V2_SCORE_BUCKETS: tuple[str, ...] = ("security", "performance", "architecture", "quality", "accessibility")
V2_SCORE_WEIGHTS: dict[str, float] = {
    "security": 2.0,
    "performance": 1.0,
    "architecture": 1.0,
    "quality": 0.8,
...
V2_CATEGORY_FALLBACKS: dict[str, str] = {
    Category.SECURITY.value: "security",
    Category.PERFORMANCE.value: "performance",
    Category.ACCESSIBILITY.value: "accessibility",
    Category.ARCHITECTURE.value: "architecture",
    Category.LARAVEL_BEST_PRACTICE.value: "architecture",
    Category.SRP.value: "architecture",
    Category.DRY.value: "quality",
    Category.COMPLEXITY.value: "quality",
    Category.MAINTAINABILITY.value: "quality",
    Category.REACT_BEST_PRACTICE.value: "quality",
    Category.VALIDATION.value: "quality",
}
...
class ScoringEngine:
    ...
    def calculate_v2_score(self, findings: list[Finding], rules_executed: list[str] | None = None) -> ScanScore:
        """Calculate the v2 rule-weighted score dashboard."""
        try:
            from core.rule_engine import RUNTIME_RULES
        except Exception:
            RUNTIME_RULES = {}

        enabled_rule_ids = list(rules_executed or [])
        ...
        for rule_id in enabled_rule_ids:
            ...
            if rule_class is None:
                continue
            bucket = self._v2_bucket_for_rule_class(rule_class)
            if bucket not in max_possible:
                continue
        ...
        seen_fired: set[tuple[str, str]] = set()
        for finding in findings:
            rule_id = str(getattr(finding, "rule_id", "") or "").strip()
            rule_class = RUNTIME_RULES.get(rule_id)
            bucket = self._v2_bucket_for_rule_class(rule_class) if rule_class is not None else self._v2_bucket_for_finding(finding)
            if bucket not in actual_penalty:
                bucket = "quality"
            fingerprint = str(getattr(finding, "fingerprint", "") or "").strip()
            ...
            actual_penalty[bucket] += classification_weight * (
                ...
                else self._v2_unknown_finding_weight(finding)
            )

        scores: dict[str, int] = {}
        for bucket in V2_SCORE_BUCKETS:
            possible = max_possible[bucket]
            penalty = actual_penalty[bucket]
            if possible <= 0:
                scores[bucket] = 0 if penalty > 0 else 100
            else:
                scores[bucket] = max(0, round(100 - (penalty / possible * 100)))

        weighted_total = sum(scores[bucket] * V2_SCORE_WEIGHTS[bucket] for bucket in V2_SCORE_BUCKETS)
        total_weight = sum(V2_SCORE_WEIGHTS.values())
        scores["overall"] = max(0, min(100, round(weighted_total / total_weight))) if total_weight > 0 else 100
    ...
    def _v2_rule_weight(self, rule_class: object | None) -> int:
        if rule_class is None:
            return V2_UNKNOWN_RULE_WEIGHT
        try:
            ...
            pass
        severity = getattr(rule_class, "default_severity", Severity.MEDIUM)
        try:
            severity_enum = severity if isinstance(severity, Severity) else Severity(str(severity))
        except Exception:
            severity_enum = Severity.MEDIUM
        return V2_SEVERITY_WEIGHTS.get(severity_enum, V2_UNKNOWN_RULE_WEIGHT)

    def _v2_unknown_finding_weight(self, finding: Finding) -> int:
        if str(getattr(finding, "rule_id", "") or "").strip():
            return V2_UNKNOWN_RULE_WEIGHT
        return V2_SEVERITY_WEIGHTS.get(getattr(finding, "severity", Severity.MEDIUM), V2_UNKNOWN_RULE_WEIGHT)

    def _v2_bucket_for_rule_class(self, rule_class: object | None) -> str:
        if rule_class is None:
            return "quality"
        tags = getattr(rule_class, "tags", None)
        if isinstance(tags, dict):
            tag_type = str(tags.get("type", "") or "").strip().lower()
            if tag_type in V2_SCORE_BUCKETS:
                return tag_type
            if tag_type == "testing":
                return "quality"
        category = getattr(rule_class, "category", Category.MAINTAINABILITY)
        category_key = category.value if isinstance(category, Category) else str(category or "").strip().lower()
        return V2_CATEGORY_FALLBACKS.get(category_key, "quality")

    def _v2_bucket_for_finding(self, finding: Finding) -> str:
        category = getattr(finding, "category", Category.MAINTAINABILITY)
        category_key = category.value if isinstance(category, Category) else str(category or "").strip().lower()
        return V2_CATEGORY_FALLBACKS.get(category_key, "quality")
    ...
    def generate_report(
        ...
        action_plan.sort(
            key=lambda a: (
                -classification_rank.get(getattr(a, "classification", FindingClassification.ADVISORY), 0),
                -a.priority,
                a.category,
                a.rule_id,
                a.id,
        ...
        triage_plan.sort(
            key=lambda t: (
                -classification_rank.get(getattr(t, "classification", FindingClassification.ADVISORY), 0),
                -t.triage_score,
                t.category,
                t.rule_id,
                t.id,
