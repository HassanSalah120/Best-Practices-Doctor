...
RULE_METADATA: list[RuleInfo] = [
    ...
    # === Additional React/Architecture Rules ===
    RuleInfo(
        id="error-pages-missing",
        name="Error Pages Missing",
        description="Missing core 4xx/5xx error pages",
        category="laravel_best_practice",
        severity="high",
        layer="backend",
    ...
    RuleInfo(
        ...
        description="Custom hook is overly complex and should be split",
        category="react_best_practice",
        severity="medium",
        layer="frontend",
    ...
    RuleInfo(
        ...
        name="Cross Feature Import Boundary",
        description="Import crosses feature module boundaries",
        category="architecture",
        severity="medium",
        layer="frontend",
        tags=["react", "architecture", "imports"],
    ),
    RuleInfo(
        id="query-key-instability",
        name="Query Key Instability",
        description="React Query key is unstable and causes refetching",
        category="performance",
        severity="medium",
        layer="frontend",
    ...
    RuleInfo(
        ...
        description="Event relay pattern detected in useEffect",
        category="react_best_practice",
        severity="low",
        layer="frontend",
        tags=["react", "useeffect", "events"],
    ),
    RuleInfo(
        id="route-shell-missing-error-boundary",
        name="Route Shell Missing Error Boundary",
        description="Route shell component lacks error boundary",
        category="react_best_practice",
        severity="high",
        layer="frontend",
        tags=["react", "routing", "errors"],
    ),
    RuleInfo(
        id="unsafe-async-handler-without-guard",
        name="Unsafe Async Handler Without Guard",
        description="Async handler lacks loading/error state guards",
        category="react_best_practice",
        severity="medium",
        layer="frontend",
        tags=["react", "async", "handlers"],
    ),
    RuleInfo(
        id="exhaustive-deps-ast",
        name="Exhaustive Deps AST",
        description="useEffect missing dependencies detected via AST",
        category="react_best_practice",
        severity="high",
        layer="frontend",
    ...
    RuleInfo(
        ...
        description="useEffect lacks cleanup function for side effects",
        category="react_best_practice",
        severity="medium",
        layer="frontend",
    ...
    RuleInfo(
        id="usememo-ast",
        name="useMemo AST",
        description="useMemo usage detected via AST analysis",
        category="react_best_practice",
        severity="low",
        layer="frontend",
    ...
    RuleInfo(
        id="usecallback-ast",
        name="useCallback AST",
        description="useCallback usage detected via AST analysis",
        category="react_best_practice",
        severity="low",
        layer="frontend",
    ...
    RuleInfo(
        ...
        description="Direct model access could use repository pattern",
        category="laravel_best_practice",
        severity="medium",
        layer="backend",
    ...
    RuleInfo(
        id="crawlable-internal-navigation-required",
        name="Crawlable Internal Navigation Required",
        description="Internal navigation not crawlable by search engines",
        category="accessibility",
        severity="medium",
        layer="frontend",
        tags=["seo", "navigation", "crawling"],
    ),
    RuleInfo(
        id="h1-singleton-violation",
        name="H1 Singleton Violation",
...
def _rule_class_map() -> dict[str, Any]:
    try:
...
def _profiles_by_rule() -> dict[str, list[str]]:
        ...
        for profile in list_profiles():
            ...
            for rule_id, config in ruleset.rules.items():
                if bool(getattr(config, "enabled", True)):
                    result.setdefault(rule_id, []).append(profile)
    except Exception:
        return result
    return result


def _severity_weight(rule_class: Any, severity: str) -> int:
    try:
        explicit = int(getattr(rule_class, "severity_weight", 0) or 0)
        if explicit > 0:
            return explicit
    except Exception:
        pass
    try:
        ...
        return severity_weight_for(severity)
    except Exception:
        return {"low": 2, "medium": 5, "high": 8, "critical": 10}.get(severity, 5)


def _structured_tags(rule: RuleInfo, rule_class: Any) -> dict[str, str]:
    raw = getattr(rule_class, "tags", None)
    if isinstance(raw, dict):
        domain = str(raw.get("domain", "") or "").strip().lower()
        tag_type = str(raw.get("type", "") or "").strip().lower()
        ...
        if module_name.startswith("rules.react."):
            ...
            domain = "laravel"
        elif module_name.startswith("rules.php."):
            domain = "php"
        else:
            domain = "general"
        tag_type = _category_to_v2_type(rule.category)
        concern = next((str(tag) for tag in rule.tags if str(tag) not in {domain, tag_type}), "")

    if domain not in {"laravel", "react", "php", "general"}:
        domain = "general"
    if tag_type not in {"security", "performance", "architecture", "quality", "accessibility", "testing"}:
        tag_type = _category_to_v2_type(rule.category)
    return {"domain": domain, "type": tag_type, "concern": concern}


def _category_to_v2_type(category: str) -> str:
    category = str(category or "").strip().lower()
    if category in {"security", "performance", "accessibility", "architecture"}:
        return category
    if category in {"tests", "testing"}:
        return "testing"
    return "quality"


def _rule_attr(rule_class: Any, name: str, default: Any) -> Any:
    value = getattr(rule_class, name, default)
    if value is None:
        return default
    return value


def _serialize_rule_v2(rule: RuleInfo, rule_class: Any, profiles_by_rule: dict[str, list[str]]) -> dict[str, Any]:
    tags_legacy = list(rule.tags or [])
    structured_tags = _structured_tags(rule, rule_class)
    severity = str(rule.severity or "medium").lower()
    return {
        "id": rule.id,
        "name": rule.name,
        "description": rule.description,
        "severity": severity,
        "severity_weight": _severity_weight(rule_class, severity),
        "confidence": str(_rule_attr(rule_class, "confidence", "medium") or "medium"),
        "fix_suggestion": str(_rule_attr(rule_class, "fix_suggestion", "") or ""),
        "examples": dict(_rule_attr(rule_class, "examples", {}) or {}),
        "priority": int(_rule_attr(rule_class, "priority", 3) or 3),
        "group": str(_rule_attr(rule_class, "group", "") or ""),
        "profiles": list(profiles_by_rule.get(rule.id, [])),
        "applies_to": list(_rule_attr(rule_class, "applies_to", []) or []),
        "references": list(_rule_attr(rule_class, "references", []) or []),
        "related_rules": list(_rule_attr(rule_class, "related_rules", []) or []),
        "false_positive_notes": str(_rule_attr(rule_class, "false_positive_notes", "") or ""),
        "detection_type": str(_rule_attr(rule_class, "detection_type", "regex") or "regex"),
        "analysis_cost": str(_rule_attr(rule_class, "analysis_cost", "low") or "low"),
        "auto_fixable": bool(_rule_attr(rule_class, "auto_fixable", False)),
...
def get_rules_grouped_for_ui() -> dict:
    ...
    for rule in serialized_rules:
        ...
        category = str((rule.get("tags") or {}).get("type", "quality")).lower()
        if category == "testing":
            category = "quality"
    ...
    summary: dict[str, Any] = {
        "canonical_rule_count": len({rule.id for rule in rules}),
        "ui_rule_count": sum(len(category["rules"]) for layer in layers_data for category in layer["categories"]),
        "discovered_rule_count": len({rule.id for rule in rules}),
    ...
    try:
        from core.rule_engine import DISCOVERED_RULES, INTERNAL_RULE_WRAPPERS, RUNTIME_RULES

        metadata_by_id = {rule.id: rule for rule in rules}
        aliases = [
            {
                "id": alias_id,
                "name": _build_fallback_rule_info(
                    alias_id,
                    DISCOVERED_RULES.get(alias_id, object),
                ).name,
                "target": target_id,
                "target_name": metadata_by_id.get(
                    target_id,
                    _build_fallback_rule_info(target_id, RUNTIME_RULES.get(target_id, object)),
                ).name,
            }
            for alias_id, target_id in sorted(INTERNAL_RULE_WRAPPERS.items())
        ]
        summary = {
            "canonical_rule_count": len(RUNTIME_RULES),
            "ui_rule_count": summary["ui_rule_count"],
            "discovered_rule_count": len(DISCOVERED_RULES),
            "internal_alias_count": len(aliases),
    ...
    return {"layers": layers_data, "summary": summary}
