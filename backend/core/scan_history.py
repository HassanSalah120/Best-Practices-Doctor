...
class ScanSummary:
    ...
    def from_report(cls, report: ScanReport, profile: str = "startup") -> ScanSummary:
        ...
        # Get category scores
        category_scores = {}
        breakdown = getattr(report, "category_breakdown", {}) or {}
        if isinstance(breakdown, dict):
            for category, cat_score in breakdown.items():
                score = getattr(cat_score, "score", None)
                if score is None:
                    score = getattr(cat_score, "raw_score", 0.0)
                category_scores[str(category)] = float(score or 0.0)
        ...
        return cls(
            ...
            timestamp=datetime.now(),
            overall_score=float(getattr(scores, "overall", 0.0) or 0.0),
            grade=str(getattr(scores, "grade", "F") or "F"),
            ...
            files_scanned=int(getattr(report, "files_scanned", 0) or len(report.file_summaries or [])),
            execution_time_ms=float(getattr(report, "duration_ms", 0) or 0.0),
            profile=profile,
        )
...
class ScanHistoryManager:
    ...
    def add_scan(self, report: ScanReport, profile: str = "startup") -> ScanSummary:
        ...
        # Add new scan, replacing same-job entries so explicit saves do not duplicate auto-saves.
        history.scans = [scan for scan in history.scans if scan.job_id != summary.job_id]
    ...
    def get_history_by_path(self, project_path: str) -> ScanHistory:
        ...
        project_hash = fast_hash_hex(str(Path(project_path).resolve()), length=16)
