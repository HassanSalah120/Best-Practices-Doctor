...
class ReportingStage:
    ...
    async def run(self, context: ScanPipelineContext, state: ScanPipelineState) -> None:
        ...
        self._apply_scan_history(state, context)
        self._apply_incremental_manifest(state, context)
        self._apply_project_map_explainer(state, context)
        self._apply_agent_rules(state, context)
    ...
    def _apply_scan_history(self, state: ScanPipelineState, context: ScanPipelineContext) -> None:
        ...
        try:
            ...
            profile_name = (
                context.request.baseline_profile
                or getattr(state.ruleset, "name", "startup")
                or "startup"
            )
            ScanHistoryManager().add_scan(state.report, profile=str(profile_name))
        except Exception as exc:
            logger.warning("[Pipeline] Failed to save scan history: %s", exc)
            state.warnings.append("reporting:scan_history")

    def _apply_incremental_manifest(self, state: ScanPipelineState, context: ScanPipelineContext) -> None:
        if state.facts is None:
            return
        try:
            from core.incremental import IncrementalScanManager

            files = list(getattr(state.facts, "files", []) or [])
            if not files:
                return
            IncrementalScanManager(context.request.project_path).update_manifest(files)
        except Exception as exc:
            logger.warning("[Pipeline] Failed to update incremental manifest: %s", exc)
            state.warnings.append("reporting:incremental_manifest")

    def _apply_project_map_explainer(self, state: ScanPipelineState, context: ScanPipelineContext) -> None:
        try:
            from core.project_map import ProjectMapArtifactStore, ProjectMapBuilder

            if state.facts is None or state.report is None:
                return

            builder = ProjectMapBuilder()
            store = ProjectMapArtifactStore()
            artifact, cache_meta = builder.build_and_cache(
                facts=state.facts,
                report=state.report,
                store=store,
            )

            summary = dict(artifact.get("explainer_summary") or {})
            meta = dict(artifact.get("meta") or {})
            if not isinstance(state.report.analysis_debug, dict):
                state.report.analysis_debug = {}
            state.report.analysis_debug["project_map"] = {
                "signature": str(meta.get("signature", "")),
                "node_count": int(meta.get("counts", {}).get("nodes", 0) or 0),
                "edge_count": int(meta.get("counts", {}).get("edges", 0) or 0),
                "truncated": bool(meta.get("truncated", False)),
                "cache_hit": bool(cache_meta.get("cache_hit", False)),
            }
            state.report.analysis_debug["project_explainer_summary"] = summary
        except Exception as exc:
            logger.warning("[Pipeline] Failed to build project map/explainer artifact: %s", exc)
            state.warnings.append("reporting:project_map")

    def _apply_agent_rules(self, state: ScanPipelineState, context: ScanPipelineContext) -> None:
        if state.report is None:
            return
        if os.environ.get("BPD_DISABLE_AGENT_RULES_AUTO_WRITE") == "1":
            return
        try:
            from core.agent_rules import AgentRulesGenerator

            result = AgentRulesGenerator().write(state.report)
            if not isinstance(state.report.analysis_debug, dict):
                state.report.analysis_debug = {}
            failed = list(result.get("failed") or [])
            state.report.analysis_debug["agent_rules"] = {
            ...
            if failed:
                state.warnings.append("reporting:agent_rules")
        except Exception as exc:
            logger.warning("[Pipeline] Failed to write agent rules pack: %s", exc)
            state.warnings.append("reporting:agent_rules")
            if state.report is not None:
                ...
                state.report.analysis_debug["agent_rules"] = {
                    ...
                    "written": [],
                    "skipped_count": 0,
                    "failed": [{"path": "*", "error": str(exc)}],
                    "warnings": ["BPD could not write the AI agent rules pack, but the scan completed."],
                }
