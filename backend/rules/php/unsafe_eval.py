...
class UnsafeEvalRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the unsafe code execution (eval/assert/preg_replace /e) risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    ...
    auto_fixable = False
    tags = {'domain': 'php', 'type': 'security', 'concern': 'unsafe-eval'}
