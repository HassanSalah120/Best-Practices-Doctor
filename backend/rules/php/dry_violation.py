...
class DryViolationRule(Rule):
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Refactor the PHP code to remove the dry violation detection pattern and keep the responsibility explicit. Add a small regression test for the affected class or function when behavior changes.'
    examples = {}
    priority = 3
    group = 'PHP Quality'
    ...
    def analyze(
        ...
        for duplicate in self._normalize_duplicates(facts.duplicates):
            ...
            if self._is_low_signal_framework_wrapper_duplicate(duplicate, unique_files):
                continue
            if self._is_low_signal_cache_wrapper_duplicate(duplicate, unique_files):
                continue
    ...
    def _is_low_signal_cache_wrapper_duplicate(
        self,
        duplicate: DuplicateBlock,
        unique_files: set[str],
    ) -> bool:
        ...
        if not snippet:
            return False
        if not any(token in snippet for token in self._LOW_SIGNAL_CACHE_SNIPPETS):
            return False
        if int(getattr(duplicate, "token_count", 0) or 0) > 120:
            return False
        if not unique_files:
            return False
        return all(
            path.startswith(("app/Repositories/", "app/Services/"))
            for path in unique_files
        )
