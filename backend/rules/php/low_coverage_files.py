...
class LowCoverageFilesRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Add focused tests around the behavior this rule protects. Prefer small regression tests that would fail before the fix.'
    examples = {}
    priority = 3
    group = 'Testing'
    applies_to = ['php-class', 'test']
    references = []
    related_rules = []
    ...
    analysis_cost = 'high'
    auto_fixable = False
    tags = {'domain': 'php', 'type': 'quality', 'concern': 'low-coverage-files'}
