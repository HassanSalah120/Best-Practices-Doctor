...
class SqlInjectionRiskRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the sql injection risk (raw sql with variables) risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 3
    group = 'Injection Risks'
    applies_to = ['php-class', 'php-function']
    references = ['OWASP A03:2021 - Injection', 'CWE-89']
    related_rules = ['raw-sql']
    false_positive_notes = ''
    detection_type = 'ast'
    analysis_cost = 'medium'
    auto_fixable = False
    tags = {'domain': 'php', 'type': 'security', 'concern': 'sql-injection'}
