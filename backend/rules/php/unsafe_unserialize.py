...
class UnsafeUnserializeRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the unsafe unserialize() usage risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    ...
    auto_fixable = False
    tags = {'domain': 'php', 'type': 'security', 'concern': 'unsafe-unserialize'}
