...
class NPlusOneRiskRule(Rule):
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Reduce the n+1 risk detection by moving expensive work out of hot paths or adding the missing cache/query optimization. Keep the behavior identical and cover the faster path with a focused test.'
    examples = {}
    priority = 2
    group = 'Performance'
    applies_to = ['global']
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'performance', 'concern': 'n-plus-one'}
    """
