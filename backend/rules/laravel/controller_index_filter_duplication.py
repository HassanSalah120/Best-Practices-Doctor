...
class ControllerIndexFilterDuplicationRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Move the controller index filter duplication responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 3
    group = 'Data Access'
    applies_to = ['controller', 'migration']
    references = []
    related_rules = ['service-extraction', 'controller-business-logic']
    false_positive_notes = ''
    detection_type = 'ast'
    analysis_cost = 'medium'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'architecture', 'concern': 'controller-index-filter'}
