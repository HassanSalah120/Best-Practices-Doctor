...
class ModelCrossModelQueryRule(Rule):
    ...
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Move the cross-model query inside model responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 4
    group = 'Data Access'
    applies_to = ['model']
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'architecture', 'concern': 'model-cross-model'}
