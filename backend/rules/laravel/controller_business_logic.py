...
class ControllerBusinessLogicRule(Rule):
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Move the business logic in controller responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 2
    group = 'Architecture Integrity'
    applies_to = ['controller']
    references = []
    related_rules = ['service-extraction']
    false_positive_notes = ''
    detection_type = 'ast'
    analysis_cost = 'medium'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'architecture', 'concern': 'controller-business-logic'}
    """
    ...
    def _looks_like_thin_orchestration(
        ...
        if not has_delegation:
            return False
        delegated_call_count = sum(
            1
            for call in call_sites
            if any(marker in call for marker in self._DELEGATION_CALL_MARKERS)
        )
        ...
        delegated_service_orchestration = (
            has_delegation
            and delegated_call_count >= 1
            and metrics.query_count <= 1
            and metrics.loop_count == 0
            and metrics.conditional_count <= 5
            and metrics.cyclomatic_complexity <= 8
            and (method.loc or 0) <= 130
        )
        if delegated_service_orchestration:
            return True
