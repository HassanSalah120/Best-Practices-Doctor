...
class UploadSizeLimitMissingRule(Rule):
    ...
    _MANUAL_SIZE_GUARD_MARKERS = (
        "assertfilesizewithinlimit(",
        "validatebaselineuploadlimit(",
        "getsiz(",
        "maxbytes",
        "maxuploadkb",
        "upload_max_size",
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the upload size limit missing risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 3
    group = 'File Security'
    applies_to = ['global']
    references = ['OWASP A05:2021 - Security Misconfiguration', 'CWE-434']
    related_rules = ['unsafe-file-upload', 'upload-mime-extension-mismatch']
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'upload-size-limit'}
    ...
    def analyze_regex(
        ...
        for match in self._UPLOAD_RULE.finditer(text):
            ...
            if self._has_explicit_max_size(rules_low):
                continue
            if self._has_manual_size_guard(text):
                continue
    ...
    def _has_manual_size_guard(self, text: str) -> bool:
        low = str(text or "").lower()
        return any(marker in low for marker in self._MANUAL_SIZE_GUARD_MARKERS)
