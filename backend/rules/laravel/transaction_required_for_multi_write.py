...
class TransactionRequiredForMultiWriteRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Move the transaction required for multi write responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 2
    group = 'Data Access'
    applies_to = ['service']
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'architecture', 'concern': 'transaction-for-multi'}
