...
class ControllerQueryDirectRule(Rule):
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Move the controller should not query db directly responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 2
    group = 'Data Access'
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'architecture', 'concern': 'controller-query-direct'}
    """
