...
class UnsafeFileUploadRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the unsafe file upload risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    group = 'File Security'
    applies_to = ['global']
    references = ['OWASP A05:2021 - Security Misconfiguration', 'CWE-434']
    related_rules = ['upload-mime-extension-mismatch']
    false_positive_notes = ''
    detection_type = 'ast'
    analysis_cost = 'medium'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'unsafe-file-upload'}
