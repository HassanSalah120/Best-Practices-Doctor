...
class InertiaSharedPropsPayloadBudgetRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Reduce the inertia shared props payload budget by moving expensive work out of hot paths or adding the missing cache/query optimization. Keep the behavior identical and cover the faster path with a focused test.'
    examples = {}
    priority = 3
    group = 'Performance'
    applies_to = ['provider']
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'performance', 'concern': 'inertia-shared-props'}
