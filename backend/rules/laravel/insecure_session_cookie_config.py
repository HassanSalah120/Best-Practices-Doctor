...
class InsecureSessionCookieConfigRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the insecure session cookie config risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'insecure-session-cookie'}
