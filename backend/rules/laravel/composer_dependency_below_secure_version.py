...
class ComposerDependencyBelowSecureVersionRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Remove the composer dependency below secure version risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    group = 'Security Hardening'
    applies_to = ['global']
    references = ['OWASP A06:2021 - Vulnerable and Outdated Components', 'CWE-1104']
    related_rules = []
    false_positive_notes = ''
    detection_type = 'regex'
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'composer-dependency-below'}
