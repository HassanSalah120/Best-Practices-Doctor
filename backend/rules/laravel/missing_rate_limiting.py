...
class MissingRateLimitingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the missing rate limiting risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    ...
    analysis_cost = 'high'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'rate-limiting'}
