...
class SensitiveResponseCacheControlMissingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the sensitive response missing cache-control risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 3
    ...
    analysis_cost = 'high'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'sensitive-response-cache'}
