...
class AssetVersioningCheckRule(Rule):
    ...
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Reduce the asset versioning check by moving expensive work out of hot paths or adding the missing cache/query optimization. Keep the behavior identical and cover the faster path with a focused test.'
    examples = {}
    priority = 4
    group = 'Performance'
    applies_to = ['global']
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'performance', 'concern': 'asset-versioning-check'}
