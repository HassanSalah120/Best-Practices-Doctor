...
class PasswordResetTokenHardeningMissingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the password reset token hardening missing risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    group = 'Sensitive Data'
    applies_to = ['global']
    references = ['OWASP A07:2021 - Identification and Authentication Failures', 'CWE-521']
    related_rules = []
    ...
    analysis_cost = 'high'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'password-reset-token'}
