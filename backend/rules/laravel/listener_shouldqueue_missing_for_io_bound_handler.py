...
class ListenerShouldQueueMissingForIoBoundHandlerRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Reduce the listener shouldqueue missing for io-bound handler by moving expensive work out of hot paths or adding the missing cache/query optimization. Keep the behavior identical and cover the faster path with a focused test.'
    examples = {}
    priority = 3
    group = 'Performance'
    applies_to = ['job']
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'performance', 'concern': 'listener-shouldqueue-for'}
