...
class ServiceExtractionRule(Rule):
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Move the service extraction suggestion responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 4
    ...
    _SERVICE_PROPERTY_CALL_RE = re.compile(
        r"\$this->(?:[a-zA-Z_][a-zA-Z0-9_]*)?(service|repository|action|coordinator|workflow)(?:[a-zA-Z0-9_]*)\s*->",
        re.IGNORECASE,
    )
    _THIS_PROPERTY_METHOD_CALL_RE = re.compile(
        r"\$this->(?P<property>[a-zA-Z_][a-zA-Z0-9_]*)\s*->(?P<method>[a-zA-Z_][a-zA-Z0-9_]*)\s*\(",
        re.IGNORECASE,
    )
    ...
    def _decision_profile(
        ...
        *,
        ...
        delegated = self._uses_action_or_service_delegation(method, has_service_injection=has_service_injection)
    ...
    def _uses_action_or_service_delegation(self, method: MethodInfo, *, has_service_injection: bool = False) -> bool:
        params = [str(param or "") for param in (method.parameters or [])]
        calls = [str(call or "") for call in (method.call_sites or [])]
        calls_low = [call.lower() for call in calls]

        # Look for delegation patterns in call sites
        delegation_patterns = [
            r"\$this->(?:service|action|repo|repository)->[a-zA-Z0-9_]+\(",
            r"\$[a-zA-Z0-9_]+Action->(?:execute|handle)",
            r"[A-Z][a-zA-Z0-9_]+::(?:run|execute|make)",
        ]
        if any(re.search(pattern, call) for call in calls for pattern in delegation_patterns):
            return True
        ...
        has_execute_call = any(any(marker in call for marker in self._DELEGATION_MARKERS) for call in calls_low)
        has_service_property_call = any(self._SERVICE_PROPERTY_CALL_RE.search(call) for call in calls)
        this_property_calls = [
            match
            for call in calls
            for match in self._THIS_PROPERTY_METHOD_CALL_RE.finditer(call)
            if match.group("method").lower() not in {"middleware", "authorize", "validate", "execute", "handle", "process", "run"}
        ]
        method_name_normalized = re.sub(r"[^a-z0-9]+", "", str(method.name or "").lower())
        overlapping_property_calls = [
            match
            for match in this_property_calls
            if method_name_normalized
            and method_name_normalized in re.sub(r"[^a-z0-9]+", "", match.group("method").lower())
        ]

        has_business_keyword = any(
            keyword in call
            for call in calls_low
            if not any(marker in call for marker in self._SAFE_CONTROLLER_CALL_MARKERS)
            for keyword in self.BUSINESS_LOGIC_PATTERNS
        )

        # Check for simple one-line delegation in a small method
        if (method.loc or 0) <= 5 and any("->" in call and "(" in call for call in calls):
            return True
        ...
        if has_service_injection and overlapping_property_calls and (method.loc or 0) <= 120:
            query_like_calls = sum(
                1
                for call in calls_low
                if any(token in call for token in ("->where", "->create", "->update", "->save", "->sync", "->attach", "->detach"))
            )
            if query_like_calls <= 1 and not has_business_keyword:
                return True
