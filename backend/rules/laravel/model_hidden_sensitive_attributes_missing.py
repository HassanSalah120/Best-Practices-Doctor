...
class ModelHiddenSensitiveAttributesMissingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Remove the model hidden sensitive attributes missing risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    ...
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'model-hidden-sensitive'}
            ...
            for config in configs:
                ...
                for value in raw_values:
                    ...
                    if any(token in value_l for token in self._SENSITIVE_TOKENS):
                        if any(value_l.endswith(suffix) for suffix in self._SAFE_NON_SECRET_SUFFIXES):
                            continue
