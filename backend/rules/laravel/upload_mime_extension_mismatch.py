...
class UploadMimeExtensionMismatchRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Remove the upload mime/extension mismatch risk risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    group = 'File Security'
    applies_to = ['global']
    references = ['OWASP A05:2021 - Security Misconfiguration', 'CWE-434']
    related_rules = ['unsafe-file-upload']
    false_positive_notes = ''
    detection_type = 'regex'
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'security', 'concern': 'upload-mime-extension'}
