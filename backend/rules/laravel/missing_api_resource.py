...
class MissingApiResourceRule(Rule):
    ...
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Move the missing api resource responsibility into the appropriate service, action, repository, or boundary object. Keep controllers and UI components focused on orchestration only.'
    examples = {}
    priority = 4
    group = 'API Design'
    applies_to = ['global']
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'laravel', 'type': 'architecture', 'concern': 'api-resource'}
