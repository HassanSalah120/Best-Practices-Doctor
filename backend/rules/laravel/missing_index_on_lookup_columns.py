...
class MissingIndexOnLookupColumnsRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Reduce the missing index on lookup columns by moving expensive work out of hot paths or adding the missing cache/query optimization. Keep the behavior identical and cover the faster path with a focused test.'
    examples = {}
    priority = 3
    group = 'Performance'
    ...
    auto_fixable = True
    tags = {'domain': 'laravel', 'type': 'performance', 'concern': 'index-on-lookup'}
    ...
    def analyze(
        ...
        for change in changes:
            if str(change.operation or "").lower() != "add_column":
                continue
            snippet = str(change.snippet or "").lower()
            # `change()` operations modify existing columns and are frequently already indexed.
