...
class UseEffectCleanupMissingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Refactor the component code to remove the useeffect cleanup missing pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 3
    group = 'React Stability'
    applies_to = ['react-component', 'hook']
    references = []
    related_rules = ['exhaustive-deps-ast']
    false_positive_notes = ''
    detection_type = 'regex'
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'useeffect-cleanup'}
