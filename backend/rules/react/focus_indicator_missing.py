...
class FocusIndicatorMissingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Update the component markup and interaction contract so the focus indicator missing is accessible by keyboard and assistive technology. Verify the expected ARIA/semantic state in a component test when practical.'
    examples = {}
    priority = 3
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'accessibility', 'concern': 'focus-indicator'}
