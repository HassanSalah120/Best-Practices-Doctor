...
class StatusMessageAnnouncementRule(Rule):
    ...
    _NON_RENDERING_FILES = [
        ...
        re.compile(r"/services?/", re.IGNORECASE),  # Transport/service modules don't render DOM
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Update the component markup and interaction contract so the status message announcement is accessible by keyboard and assistive technology. Verify the expected ARIA/semantic state in a component test when practical.'
    examples = {}
    priority = 3
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'accessibility', 'concern': 'status-message-announcement'}
