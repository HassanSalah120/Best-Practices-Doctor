...
class DangerousHtmlSinkWithoutSanitizerRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Remove the dangerous html sink without sanitizer risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    group = 'Injection Risks'
    applies_to = ['react-component']
    references = ['OWASP A05:2021 - Security Misconfiguration']
    related_rules = ['no-dangerously-set-inner-html']
    false_positive_notes = ''
    detection_type = 'ast'
    analysis_cost = 'medium'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'security', 'concern': 'dangerous-html-sink'}
