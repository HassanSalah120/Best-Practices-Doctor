...
class NoInlineTypesRule(Rule):
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Extract only shared or numerous component-local types when doing so reduces coupling. Keep single-use local contracts beside the component when extraction would add indirection.'
    examples = {}
    priority = 3
    group = 'Code Quality'
    ...
    references = []
    related_rules = []
    false_positive_notes = 'Single-use local types, React context contracts, Props/State types, and ambient Window declarations are usually acceptable colocated with the code they describe.'
    detection_type = 'regex'
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'inline-types'}
    """
    ...
    def analyze(
        ...
        for comp in facts.react_components:
            ...
            if self._is_context_contract_file(comp.file_path, comp.name, type_names):
                continue
            if len(type_names) < 2:
                continue
            if not type_names:
    ...
    def analyze_regex(
        ...
        if self._is_context_contract_file(file_path, "", type_names):
            return []
        if len(type_names) < 2:
            return []
        if not type_names:
    ...
    @staticmethod
    def _is_context_contract_file(file_path: str, component_name: str, type_names: list[str]) -> bool:
        stem = os.path.splitext(os.path.basename(file_path.lower().replace("\\", "/")))[0]
        if "context" in stem:
            return True
        if component_name.endswith("Provider") and any(name.endswith("ContextType") for name in type_names):
            return True
        return any(name.endswith("ContextType") for name in type_names)
