...
class TypeScriptTypeCheckRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Refactor the component code to remove the typescript type check pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 2
    group = 'Code Quality'
    ...
    analysis_cost = 'high'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'typescript-type-check'}
