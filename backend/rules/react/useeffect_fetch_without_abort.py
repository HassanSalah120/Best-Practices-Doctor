...
class UseEffectFetchWithoutAbortRule(Rule):
    ...
    severity_weight = 0
    confidence = 'high'
    fix_suggestion = 'Refactor the component code to remove the useeffect fetch without abort pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 3
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'react-useeffect-fetch'}
