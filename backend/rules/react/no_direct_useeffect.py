...
class NoDirectUseEffectRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Refactor the component code to remove the direct useeffect is disallowed pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 3
    group = 'React Stability'
    applies_to = ['react-component', 'hook']
    references = []
    related_rules = ['useeffect-cleanup-missing', 'exhaustive-deps-ast']
    false_positive_notes = 'Dedicated custom hooks that synchronize with external systems such as WebSockets and perform cleanup are allowed.'
    detection_type = 'ast'
    analysis_cost = 'medium'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'direct-useeffect'}
    ...
    def analyze_ast(
        ...
        for call in self._find_effect_calls(tree.root_node, content_bytes):
            ...
            if reason == "external-sync" and self._is_safe_external_sync_effect(call):
                continue
    ...
    def _is_safe_external_sync_effect(self, call: _EffectCall) -> bool:
        body = str(call.callback_text or "")
        low = body.lower()
        has_cleanup = "return () =>" in body or "return function" in low
        if not has_cleanup:
            return False
        ...
        return any(token in low for token in external_tokens)
