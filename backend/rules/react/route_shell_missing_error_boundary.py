...
class RouteShellMissingErrorBoundaryRule(Rule):
    ...
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Refactor the component code to remove the route shell missing error boundary pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 4
    group = 'Code Quality'
    applies_to = ['react-component', 'page', 'layout']
    references = []
    related_rules = []
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'route-shell-error'}
