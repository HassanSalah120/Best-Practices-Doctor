...
class NoDangerouslySetInnerHtmlRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Remove the no dangerously set inner html risk and enforce the relevant Laravel/React security control at the boundary. Add a regression test that proves unsafe input or configuration is rejected.'
    examples = {}
    priority = 1
    group = 'Injection Risks'
    applies_to = ['react-component']
    references = ['OWASP A05:2021 - Security Misconfiguration']
    related_rules = ['dangerous-html-sink-without-sanitizer']
    false_positive_notes = ''
    detection_type = 'regex'
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'security', 'concern': 'dangerously-set-inner'}
