...
class ReactParentChildSpacingOverlapRule(Rule):
    ...
    severity_weight = 0
    confidence = 'medium'
    fix_suggestion = 'Refactor the component code to remove the parent/child spacing overlap pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 3
    group = 'Code Quality'
    ...
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'react-parent-child'}
    ...
    def analyze_ast(
        ...
        # High-signal only: focus on pages/screens to avoid noise in low-level components
        normalized_path = str(file_path or "").lower().replace("\\", "/")
        segments = [s for s in normalized_path.split("/") if s]
        if not any(marker in segments for marker in self._PAGE_PATH_MARKERS):
            return []
