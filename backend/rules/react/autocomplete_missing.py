...
class AutocompleteMissingRule(Rule):
    ...
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Update the component markup and interaction contract so the autocomplete attribute missing is accessible by keyboard and assistive technology. Verify the expected ARIA/semantic state in a component test when practical.'
    examples = {}
    priority = 4
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'accessibility', 'concern': 'autocomplete'}
