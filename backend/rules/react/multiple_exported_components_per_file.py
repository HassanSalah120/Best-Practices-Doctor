...
class MultipleExportedComponentsPerFileRule(Rule):
    ...
    severity_weight = 0
    confidence = 'low'
    fix_suggestion = 'Refactor the component code to remove the multiple exported react components pattern while preserving the public UI behavior. Prefer explicit state, props, and lifecycle boundaries over implicit side effects.'
    examples = {}
    priority = 4
    group = 'Code Quality'
    ...
    analysis_cost = 'low'
    auto_fixable = False
    tags = {'domain': 'react', 'type': 'quality', 'concern': 'multiple-exported-react'}
    ...
    def analyze_regex(
        ...
        if self._share_file_prefix(file_path, names):
            return []
        if self._share_file_suffix(file_path, names):
            return []
    ...
    @classmethod
    def _share_file_suffix(cls, file_path: str, names: list[str]) -> bool:
        stem = (file_path or "").replace("\\", "/").split("/")[-1].split(".")[0]
        stem_suffix = "".join(cls._split_tokens(stem)).lower()
        if stem_suffix and all(name.lower().endswith(stem_suffix) for name in names):
            return True
        ...
        suffix_text = "".join(reversed(shared_suffix)).lower()
        return len(suffix_text) >= 4
