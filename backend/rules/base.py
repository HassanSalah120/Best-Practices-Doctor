...
def severity_weight_for(severity: Severity | str | None) -> int:
    """Return the v2 rule metadata weight for a severity value."""
    raw = severity.value if isinstance(severity, Severity) else str(severity or "").strip().lower()
    return SEVERITY_WEIGHT_MAP.get(raw, SEVERITY_WEIGHT_MAP["medium"])
...
class Rule(ABC):
    ...
    analysis_cost: str = "low"
    auto_fixable: bool = False
    tags: dict = {"domain": "general", "type": "quality", "concern": ""}
    # Rule execution type:
