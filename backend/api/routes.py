...
class ScanRequest(BaseModel):
    ...
    runtime_contract_mode: str = "hybrid"
    runtime_route_scope: str = "all"
    runtime_base_url: str | None = None
    runtime_allow_mutating_probes: bool = False
    runtime_manual_routes: list[str] | None = None
...
class ProjectChangesRequest(BaseModel):
    """Request payload for changed-files scan preflight."""
    path: str
...
def _resolve_project_file(project_path: str, rel_path: str) -> Path:
...
def _collect_project_files(project_path: Path) -> list[str]:
    try:
        import pathspec

        spec = pathspec.PathSpec.from_lines("gitwildmatch", settings.default_ignore_patterns)
        should_ignore = lambda rel: spec.match_file(rel.replace("\\", "/"))  # noqa: E731
    except Exception:
        ignore_patterns = ("/vendor/", "/node_modules/", "/bower_components/", "/.git/", "/storage/", "/bootstrap/cache/", "/public/dist/assets/")
        should_ignore = lambda rel: any(p in rel.replace("\\", "/") for p in ignore_patterns)  # noqa: E731
...
async def detect_project_changes(request: ProjectChangesRequest):
    """Detect changed files for a project before starting a differential scan."""
    path = Path(request.path)
    if not path.exists():
        raise HTTPException(status_code=400, detail=f"Path does not exist: {request.path}")
    if not path.is_dir():
    ...
    try:
        ...
        files = [str(f).replace("\\", "/") for f in (request.files or []) if str(f).strip()]
        if not files:
            files = _collect_project_files(project_path)

        manager = IncrementalScanManager(project_path)
        changes = manager.detect_changes(files)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to detect project changes: {e}")
...
async def run_scan(
    ...
    runtime_contract_mode: str,
    runtime_route_scope: str,
    runtime_base_url: str | None,
    runtime_allow_mutating_probes: bool,
    runtime_manual_routes: list[str] | None,
    job_id: str,
    ...
    pipeline_request = ScanPipelineRequest(
        ...
        runtime_route_scope=runtime_route_scope,
        runtime_base_url=runtime_base_url,
        runtime_allow_mutating_probes=runtime_allow_mutating_probes,
        runtime_manual_routes=runtime_manual_routes,
    )
...
async def start_scan(request: ScanRequest):
    ...
    await job_manager.start_job(
        ...
        request.runtime_contract_mode,
        request.runtime_route_scope,
...
@router.get("/scan/{job_id}/runtime-contracts", response_model=RuntimeContractSummary)
async def get_runtime_contracts(job_id: str):
    """Get Runtime Contract Guard summary for a completed scan."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")
    return report.runtime_contracts or RuntimeContractSummary(mode="off")


@router.get("/scan/{job_id}/runtime-contracts/tests")
async def get_runtime_contract_tests(job_id: str):
    """Get generated Runtime Contract Guard test artifacts."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")
    summary = report.runtime_contracts or RuntimeContractSummary(mode="off")
    tests = summary.generated_test_items
    return {
        "tests": [test.model_dump() for test in tests],
        "total": len(tests),
        "generated_tests": summary.generated_tests,
    }


@router.get("/scan/{job_id}/agent-rules")
async def get_scan_agent_rules(job_id: str):
    """Preview generated AI-agent project rule files for a completed scan."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")

    try:
        from core.agent_rules import AgentRulesGenerator

        return AgentRulesGenerator().preview(report)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to build agent rules preview: {e}")


@router.get("/scan/{job_id}/agent-rules/download.zip")
async def download_scan_agent_rules_zip(job_id: str):
    """Download generated AI-agent project rule files as a path-preserving ZIP archive."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")
    ...
    try:
        ...
        with zipfile.ZipFile(archive, mode="w", compression=zipfile.ZIP_DEFLATED) as zip_file:
            for file_info in preview.get("files", []):
                archive_path = str(file_info.get("path") or "").replace("\\", "/").lstrip("/")
                parts = archive_path.split("/")
                if not archive_path or any(part in {"", ".", ".."} for part in parts):
                    raise ValueError(f"Unsafe agent rules archive path: {archive_path}")
                zip_file.writestr(archive_path, str(file_info.get("content") or ""))

        safe_job_id = "".join(ch if ch.isalnum() or ch in {"-", "_"} else "-" for ch in str(report.id or job_id))
        archive.seek(0)
        return Response(
            content=archive.getvalue(),
            media_type="application/zip",
            headers={
                "Content-Disposition": f'attachment; filename="bpd-agent-rules-pack-{safe_job_id}.zip"',
            },
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to build agent rules ZIP: {e}")


@router.post("/scan/{job_id}/agent-rules/write")
async def write_scan_agent_rules(
    job_id: str,
    dry_run: bool = Query(default=False, description="Preview create/update/skip actions without writing files"),
):
    """Write or regenerate AI-agent project rule files for a completed scan."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")
    ...
    try:
        ...
        result = AgentRulesGenerator().write(report, dry_run=dry_run)
        if dry_run:
            return result
        if isinstance(report.analysis_debug, dict):
            report.analysis_debug["agent_rules"] = {
                "write_status": result.get("write_status", "unknown"),
        ...
        raise HTTPException(status_code=500, detail=f"Failed to write agent rules: {e}")
...
def _load_project_map_artifact_for_report(report: ScanReport) -> dict:
    from core.project_map import (
        ProjectMapArtifactStore,
        build_minimal_artifact_from_report,
    )

    store = ProjectMapArtifactStore()
    artifact = store.load_by_scan(project_path=report.project_path, scan_id=report.id)
    if isinstance(artifact, dict):
        return artifact
    return build_minimal_artifact_from_report(report)


@router.get("/scan/{job_id}/project-map")
async def get_project_map(job_id: str):
    """Return project intelligence map with lightweight explainer summary."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")

    artifact = _load_project_map_artifact_for_report(report)
    return {
        "nodes": list(artifact.get("nodes") or []),
        "edges": list(artifact.get("edges") or []),
        "hierarchy": dict(artifact.get("hierarchy") or {}),
        "insights": dict(artifact.get("insights") or {}),
        "explainer": dict(artifact.get("explainer_summary") or {}),
        "meta": dict(artifact.get("meta") or {}),
    }


@router.get("/project-map")
async def get_latest_project_map():
    """Return project map for the latest completed scan."""
...
@router.get("/scan/{job_id}/project-explainer")
async def get_project_explainer(
    job_id: str,
    entry_type: str | None = Query(default=None),
    entry_id: str | None = Query(default=None),
    framework: str | None = Query(default=None),
    problems_only: bool = Query(default=False),
    include_reverse: bool = Query(default=False),
):
    """Return deep project explainer payload with optional filters."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")

    artifact = _load_project_map_artifact_for_report(report)
    from core.project_map import filter_explainer_payload

    filtered = filter_explainer_payload(
        artifact,
    ...
    return {"job_id": job_id, **filtered}


@router.get("/project-explainer")
async def get_latest_project_explainer(
    entry_type: str | None = Query(default=None),
    entry_id: str | None = Query(default=None),
    framework: str | None = Query(default=None),
    problems_only: bool = Query(default=False),
    ...
    """Return deep explainer for the latest completed scan."""
    report = _latest_completed_report()
    if report is None:
        raise HTTPException(status_code=404, detail="No completed scan available")
    ...
    return {"job_id": report.id, **filtered}
...
async def add_suppression(job_id: str, request: SuppressionRequest):
    ...
    try:
        ...
        if request.fingerprint:
            finding = next(
                (f for f in report.findings if str(getattr(f, "fingerprint", "")) == request.fingerprint),
                None,
            )
            if finding is None:
                raise HTTPException(status_code=404, detail=f"Finding not found: {request.fingerprint}")

        rule_id = (request.rule_id or "").strip()
        file_pattern = (request.file_pattern or request.file or "").strip()
        line_start = request.line_start
        line_end = request.line_end
        if finding is not None:
            rule_id = rule_id or finding.rule_id
            file_pattern = file_pattern or finding.file
            line_start = line_start if line_start is not None else finding.line_start
...
@router.get("/scan/{job_id}/fixes/history")
async def list_fix_history(job_id: str):
    """List applied Auto-Fix history entries for this scan."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")

    try:
        from core.fix_history import FixHistoryManager

        entries = FixHistoryManager(report.project_path).list_entries(job_id=job_id)
        return {
            "entries": [entry.to_dict(include_content=False) for entry in entries],
            "total": len(entries),
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to list fix history: {e}")


@router.post("/scan/{job_id}/fixes/history/{entry_id}/undo")
async def undo_fix(job_id: str, entry_id: str):
    """Undo a previously applied Auto-Fix entry if the file still matches."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")
    ...
    try:
        ...
        manager = FixHistoryManager(report.project_path)
        entry = manager.get_entry(entry_id)
        if entry.job_id != job_id:
            raise HTTPException(status_code=404, detail=f"Fix history entry not found: {entry_id}")
        entry = manager.undo(entry_id)
        return {
            "status": "undone",
            "entry": entry.to_dict(include_content=False),
        }
    except HTTPException:
        raise
    except KeyError:
        raise HTTPException(status_code=404, detail=f"Fix history entry not found: {entry_id}")
    except FixHistoryConflictError as e:
        raise HTTPException(status_code=409, detail=str(e))
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=f"File not found for fix history entry: {e}")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to undo fix: {e}")


@router.post("/scan/{job_id}/fixes/history/{entry_id}/redo")
async def redo_fix(job_id: str, entry_id: str):
    """Redo a previously undone Auto-Fix entry if the file still matches."""
    report = job_manager.get_report(job_id)
    if not report:
        raise HTTPException(status_code=404, detail=f"Report not found: {job_id}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to redo fix: {e}")
...
async def apply_fix(job_id: str, file_path: str, line_start: int = Query(...), dry_run: bool = Query(True)):
    ...
    try:
        ...
        if not dry_run:
            ...
            history_entry = FixHistoryManager(report.project_path).record_apply(
                job_id=job_id,
                file=file_path,
                line_start=line_start,
                rule_id=finding.rule_id,
        ...
        return {
            ...
            "history_entry": history_entry.to_dict(include_content=False) if history_entry else None,
        }
    except HTTPException:
        raise
    except Exception as e:
