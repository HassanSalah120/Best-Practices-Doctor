...
class FactsBuilder:
    ...
    def _extract_ts_method(self, method_node, class_info, content):
        ...
        imports_used = [
            ...
            if item.file_path == class_info.file_path
        ]
    ...
    def _extract_routes(self, file_path: str, content: str):
        ...
        for stmt_start, _stmt_end, stmt in statements:
            ...
            route_matches = list(route_call.finditer(stmt))
            match_matches = list(route_match.finditer(stmt))
            use_local_fragments = len(route_matches) + len(match_matches) > 1
            for m in route_matches:
                fragment = self._route_call_fragment(stmt, m.start()) if use_local_fragments else stmt
                route_entries.append(((m.group("verb") or "").upper(), (m.group("uri") or "").strip(), fragment, m.start()))
            ...
            for m in match_matches:
                ...
                fragment = self._route_call_fragment(stmt, m.start()) if use_local_fragments else stmt
                for meth in methods:
                    route_entries.append((str(meth).upper(), uri, fragment, m.start()))

            if not route_entries:
                continue

            # Ignore group declarations and non-route helper chains.
            if ("->group(" in stmt_l or "route::group(" in stmt_l) and all(m not in {"GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS", "ANY"} for m, _, _, _ in route_entries):
                continue
            ...
            for verb, uri, route_fragment, route_offset in route_entries:
                controller, action = self._extract_route_handler(route_fragment)
                route_name = self._extract_route_name(route_fragment)
                inline_mw = self._extract_route_middleware(route_fragment)
                inline_prefixes = self._extract_route_prefixes(route_fragment)
                middleware = self._merge_route_tokens(inherited_mw, inline_mw)
                prefixes = [*inherited_prefixes, *inline_prefixes]
                line_number = content.count("\n", 0, stmt_start + route_offset) + 1
    ...
    def _route_call_fragment(self, text: str, start: int) -> str:
        """Return a single route call chain from a grouped route statement."""
