...
def test_token_storage_allows_client_generated_namespaced_session_id():
    ...
    facts = _facts_with_capabilities(mixed_public_dashboard=True)
    content = """
const sessionKey = 'chatduell_session_id';
let sessionId = localStorage.getItem(sessionKey);
if (!sessionId) {
  sessionId = crypto.randomUUID();
  localStorage.setItem('chatduell_session_id', sessionId);
}
"""
    compiled = "var k='chatduell_session_id';localStorage.setItem(k,crypto.randomUUID());"

    assert rule.analyze_regex("frontend/src/pages/Home.tsx", content, facts) == []
    assert rule.analyze_regex("public/dist/assets/Home-CD5o7rwx.js", compiled, facts) == []
...
def test_token_storage_still_flags_exact_server_session_id_storage():
    ...
    assert len(rule.analyze_regex("frontend/src/pages/Login.tsx", invalid, facts)) == 1
