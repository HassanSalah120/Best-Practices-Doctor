...
def test_all_discovered_non_runtime_rules_are_explicitly_wrapped():
    discovered_only = set(DISCOVERED_RULES.keys()) - set(ALL_RULES.keys())
    assert discovered_only == set(WRAPPED_INTERNAL_RULES.keys())
    assert get_unaccounted_discovered_rule_ids(
...
def test_rule_registry_drift_report_is_consistent():
    ...
    assert set(drift["wrapped_internal"]) == set(WRAPPED_INTERNAL_RULES.keys())
    assert drift["wrapped_internal_count"] == len(WRAPPED_INTERNAL_RULES)
    assert set(drift["legacy_alias_only"]) == set(LEGACY_RULE_ALIASES.keys()) - set(DISCOVERED_RULES.keys())
