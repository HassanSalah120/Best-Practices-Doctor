...
def test_service_extraction_skips_controller_method_delegating_to_injected_service_property():
    facts = Facts(project_path=".")
    controller = _controller("CampaignController", "app/Http/Controllers/Clinic/CampaignController.php")
    facts.controllers.append(controller)
        ...
        [
            MethodInfo(
                ...
                parameters=[
                    ...
                    "BulkEmailServiceInterface $emailService",
                    "TrustedRedirector $redirector",
                ],
            ),
            MethodInfo(
                name="trackClick",
                ...
                line_end=115,
                loc=21,
                parameters=["string $token", "CampaignTrackClickRequest $request"],
                call_sites=[
                    "$request->validated()",
                    "$this->campaigns->trackClickAndGetRedirectUrl($token, $url, $this->resolveTenantClinicId())",
                    "$this->sanitizeRedirectUrl($request, $normalizedUrl)",
                    "return redirect()->away($redirectUrl)",
                ],
            ),
        ]
