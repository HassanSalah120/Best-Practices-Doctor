...
def test_csrf_missing_on_mutating_route():
    """POST route outside routes/web.php without web middleware should be flagged."""
    facts = Facts(project_path=".")
    facts.routes.append(
        RouteInfo(
            method="POST",
...
def test_csrf_routes_web_php_keeps_group_level_csrf_with_explicit_route_middleware():
    """Per-route middleware in web.php does not remove Laravel's inherited web/CSRF group."""
    facts = Facts(project_path=".")
    for line_number, uri, middleware in [
        (53, "/api/v1/rooms", ["admin", "throttle:rooms.create", "idempotent"]),
        (55, "/api/v1/rooms/join", ["admin", "throttle:rooms.join", "idempotent"]),
        (65, "/api/v1/admin/users", ["idempotent"]),
        (73, "/api/v1/admin/rooms/{id}/players", ["auth:sanctum", "admin", "throttle:rooms.join", "idempotent"]),
    ]:
        facts.routes.append(
            RouteInfo(
                method="POST",
