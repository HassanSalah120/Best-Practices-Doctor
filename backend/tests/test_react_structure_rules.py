...
def test_multiple_exported_react_components_skips_related_suffix_variants():
    rule = MultipleExportedComponentsPerFileRule(RuleConfig())
    facts = Facts(project_path=".")
    content = """
...
"""
    ...
    findings = rule.analyze_regex("frontend/src/components/Skeleton.tsx", content, facts)
    assert findings == []


def test_no_inline_hooks_skips_context_accessor_hook():
    rule = NoInlineHooksRule(RuleConfig())
    facts = Facts(project_path=".")
    content = """
...
"""
    ...
    findings = rule.analyze_regex("frontend/src/lib/AuthContext.tsx", content, facts)
    assert findings == []
