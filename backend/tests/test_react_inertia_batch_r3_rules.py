...
{sidebar.tips.map((tip) => (
  <div key={`tip-${tip.replace(/\\s+/g, '-').toLowerCase()}-${Math.random().toString(36).substr(2, 9)}`}>
    {tip}
  </div>
))}
"""
    template_findings = rule.analyze_regex("resources/js/pages/Patients/Create.tsx", template_invalid, facts)
    assert len(template_findings) == 1
    assert template_findings[0].rule_id == "react-no-random-key"

    unrelated_random = """
{users.map((user) => <li key={user.id}>{user.name}</li>)}
const randomSeed = Math.random();
"""
    assert rule.analyze_regex("resources/js/pages/Users.tsx", unrelated_random, facts) == []
