...
def test_runtime_contract_endpoints(client, auth_headers, fixture_path):
    project_path = str(fixture_path / "sample-lara")

    start = client.post(
        "/api/scan",
        json={"path": project_path, "runtime_contract_mode": "static"},
        headers=auth_headers,
    )
    assert start.status_code == 200
    job_id = start.json()["job_id"]
    ...
    assert report.get("runtime_contracts", {}).get("mode") == "static"

    contracts_resp = client.get(f"/api/scan/{job_id}/runtime-contracts", headers=auth_headers)
    assert contracts_resp.status_code == 200
    contracts = contracts_resp.json()
    assert contracts.get("mode") == "static"
    assert "issues" in contracts
    assert "generated_test_items" in contracts

    tests_resp = client.get(f"/api/scan/{job_id}/runtime-contracts/tests", headers=auth_headers)
    assert tests_resp.status_code == 200
    tests_payload = tests_resp.json()
    assert "tests" in tests_payload
    assert "generated_tests" in tests_payload
