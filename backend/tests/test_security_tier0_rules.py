...
def test_client_side_auth_only_skips_admin_link_with_backend_admin_route():
    rule = ClientSideAuthOnlyRule(RuleConfig())
    facts = Facts(project_path=".")
    facts.routes.append(
        RouteInfo(
...
"""
    ...
    assert rule.analyze_regex("resources/js/react/pages/GamePage.tsx", content, facts) == []


def test_client_side_auth_only_skips_role_badge_and_navigation_only_admin_ui():
    rule = ClientSideAuthOnlyRule(RuleConfig())
    facts = Facts(project_path=".")
    role_badge = """
export function Profile({ user }) {
  const isAdmin = user?.role === 'admin';
  return <Badge>{isAdmin ? 'Admin' : 'User'}</Badge>;
}
"""
    nav_only = """
export function Profile({ user }) {
...
"""
    ...
    assert rule.analyze_regex("frontend/src/pages/Profile.tsx", role_badge, facts) == []
    assert rule.analyze_regex("frontend/src/pages/Profile.tsx", nav_only, facts) == []


def test_client_side_auth_only_still_flags_destructive_client_only_action():
    rule = ClientSideAuthOnlyRule(RuleConfig())
    facts = Facts(project_path=".")
    content = """
export function Profile() {
  return isAdmin && <DeleteUserButton userId={user.id} />;
}
"""

    assert len(rule.analyze_regex("frontend/src/pages/Profile.tsx", content, facts)) == 1
