...
def test_wcag_apg_rules_enabled_in_all_profiles() -> None:
    ...
    strict_only_advisory_ids = {"tailwind-motion-reduce-missing", "tailwind-appearance-none-risk"}
    ...
    for profile_name in ("startup", "balanced", "strict"):
        ...
        for rule_id in required_ids:
            ...
            if rule_id in strict_only_advisory_ids and profile_name != "strict":
...
@pytest.mark.skipif(not AST_READY, reason="Tree-sitter JSX parser is unavailable")
def test_modal_trap_focus_skips_shared_modal_consumers() -> None:
    rule = ModalTrapFocusRule(RuleConfig())
    facts = Facts(project_path=".")
    content = """
...
"""
    assert rule.analyze_ast("resources/js/pages/Portal/Users/components/Modals/EditRoleModal.tsx", content, facts) == []


@pytest.mark.skipif(not AST_READY, reason="Tree-sitter JSX parser is unavailable")
def test_dialog_focus_restore_skips_shared_modal_consumers() -> None:
    rule = DialogFocusRestoreMissingRule(RuleConfig())
    facts = Facts(project_path=".")
    content = """
import Modal from '@/components/UI/Modal';

export function EmailBookingModal() {
  return (
    <Modal open={open} onClose={onClose}>
      <div>Email booking</div>
    </Modal>
  );
}
"""
    assert rule.analyze_ast("resources/js/components/Booking/EmailBookingModal.tsx", content, facts) == []


@pytest.mark.skipif(not AST_READY, reason="Tree-sitter JSX parser is unavailable")
def test_modal_trap_focus_skips_transitive_shared_modal_consumers() -> None:
    project_root = Path(__file__).parent / "fixtures" / "dialog-trace"
    rule = ModalTrapFocusRule(RuleConfig())
    facts = Facts(project_path=str(project_root))
    content = """
import { ImportModal } from './components/ImportModal';

export function InventoryScannerContent() {
  return <ImportModal open={open} onClose={onClose} />;
}
"""
    assert (
        rule.analyze_ast(
...
@pytest.mark.skipif(not AST_READY, reason="Tree-sitter JSX parser is unavailable")
def test_dialog_focus_restore_skips_transitive_shared_modal_consumers() -> None:
    project_root = Path(__file__).parent / "fixtures" / "dialog-trace"
    rule = DialogFocusRestoreMissingRule(RuleConfig())
    facts = Facts(project_path=str(project_root))
    content = """
import BrandingCropModal from './Branding/BrandingCropModal';

export default function Branding() {
  return <BrandingCropModal open={open} onClose={onClose} />;
}
"""
    assert rule.analyze_ast("resources/js/pages/Clinic/Settings/Branding.tsx", content, facts) == []


def test_tailwind_motion_reduce_missing_skips_explicit_motion_reduce_guard() -> None:
    rule = TailwindMotionReduceMissingRule(RuleConfig())
    facts = Facts(project_path=".")
    content = """
export function Button() {
  return <button className="transition duration-200 motion-reduce:transition-none">Save</button>;
}
"""
    assert rule.analyze_regex("resources/js/components/UI/Button.tsx", content, facts) == []
