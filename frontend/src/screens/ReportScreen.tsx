...
function createAgentRulesReadFirstBlock(): PromptBlock {
    return {
        ...
        title: "Read First",
        content: [
            "Before editing, read the project AI-agent instruction files generated by Best Practices Doctor:",
            "`AGENTS.md`",
            "`RULES.md`",
            "`SKILLS.md`",
...
function promptLaneForFinding(finding: Finding): "Must Fix" | "Should Review" | "Advisory" {
    if (finding.classification === "advisory") return "Advisory";
    if (finding.severity === Severity.CRITICAL || finding.severity === Severity.HIGH) return "Must Fix";
...
function createStandardPromptConstraints(extra: string[] = []): PromptBlock {
    return {
        ...
        content: [
            ...
            "Preserve existing behavior unless the finding is the defect.",
...
function createStandardPromptOutput(extra: string[] = []): PromptBlock {
    return {
        ...
        content: [
            "Patch diff per file",
            "Brief rationale per rule or change",
            "False positives with evidence, if any",
            "Test coverage notes and commands run",
...
export const ReportScreen: React.FC<ReportScreenProps> = ({
    ...
    const sortAndFilterFindings = (findings: Finding[]) => {
        return findings
            .filter((f) => {
    ...
    }, [report?.findings, acknowledgedFingerprints, showNewOnly, newIssuesMeta.set, hideSuggestions, minSeverityRank, severityRank, ruleMetadataMap, confidenceFilter, priorityFilter, groupFilter, findingSort]);
    ...
    }, [fileFindings, acknowledgedFingerprints, showNewOnly, newIssuesMeta.set, hideSuggestions, minSeverityRank, severityRank, ruleMetadataMap, confidenceFilter, priorityFilter, groupFilter, findingSort]);
    ...
    const buildPromptForSelectedFile = () => {
        ...
        const allFileFindings = groupedFindings.flatMap((group) => group);
        const sampleFinding = groupedFindings[0]?.[0];
        const findings = groupedFindings.flatMap((group, idx) => {
            const finding = group[0];
            if (!finding) return [];
            const lines = group.length === 1
                ? `L${finding.line_start}`
                : group.map((item) => `L${item.line_start}`).join(", ");
            return `${idx + 1}. [${finding.severity}] ${finding.rule_id} at ${lines}: ${finding.title}${finding.suggested_fix ? `\n   Fix: ${compact(finding.suggested_fix, 180)}` : ""}`;
        });
        ...
        return buildPrompt([
            createPromptContext({
                ...
                summary: [`File: ${relFile}`, `Findings: ${groupedFindings.length}`],
            }),
            {
                type: "instruction",
                content: [
            ...
            { type: "findings", title: "File Findings", content: findings },
    ...
    const buildPromptForFindingGroup = (group: Finding[]) => {
        ...
        return buildPrompt([
            createPromptContext({
                ...
                architecture: inferPromptArchitectureFromFindings(group, detectedArchitecture),
                summary: [`Rule: ${finding.rule_id}`, `File: ${relFile}`, `Location: ${linePart}`],
            }),
            {
                type: "instruction",
                content: [
            ...
            },
            {
                type: "findings",
                content: [
                    `Issue: ${compact(finding.description, 220)}`,
                    finding.why_it_matters ? `Impact: ${compact(finding.why_it_matters, 180)}` : "",
                ],
            },
    ...
    const buildPromptForRule = (ruleId: string) => {
        ...
        const files = Object.keys(byFile)
            ...
            .map((file) => {
                ...
                return `${displayPath(file)}: ${lines_}`;
            });
        ...
        return buildPrompt([
            ...
            { type: "findings", title: "Files And Lines", content: files },
            createStandardPromptConstraints(["Apply the same pattern consistently across every listed file."]),
            { type: "verification", content: ["Add or update tests for behavior changes.", "Run the full test suite after the rule is fixed.", "Re-scan if available to verify the rule is cleared."] },
            createStandardPromptOutput(),
        ]);
    };
    ...
    const collectFindingsForRuleIds = (ruleIds: string[]) => {
        ...
        for (const ruleId of ruleIds) {
            const filteredMatches = (filteredReportFindings ?? []).filter((finding) => finding.rule_id === ruleId);
            const allMatches = (report.findings ?? []).filter((finding) => finding.rule_id === ruleId);
            const source = filteredMatches.length > 0 ? filteredMatches : allMatches;
    ...
    const buildMultiRulePrompt = (options: {
        ...
        const laneRank: Record<ReturnType<typeof promptLaneForFinding>, number> = {
            "Must Fix": 0,
            "Should Review": 1,
            Advisory: 2,
        };
        const sortedRuleEntries = Object.entries(byRule).sort(([, a], [, b]) => {
            const laneDelta = laneRank[promptLaneForFinding(a.sample)] - laneRank[promptLaneForFinding(b.sample)];
            if (laneDelta !== 0) return laneDelta;
            const severityDelta = (severityRank[b.sample.severity] ?? 0) - (severityRank[a.sample.severity] ?? 0);
        ...
        (["Must Fix", "Should Review", "Advisory"] as const).forEach((lane) => {
            const entries = sortedRuleEntries.filter(([, data]) => promptLaneForFinding(data.sample) === lane);
            if (entries.length === 0) return;
            const laneLines = [`### ${lane}`];
            entries.forEach(([ruleId, data]) => {
                const fileCount = new Set(data.findings.map((finding) => finding.file)).size;
                laneLines.push(
                    `- [${data.sample.severity}] ${ruleId}: ${data.findings.length} finding(s), ${fileCount} file(s)\n  Pattern: ${data.sample.title}${
                        data.sample.suggested_fix ? `\n  Expected fix: ${compact(data.sample.suggested_fix, 220)}` : ""
                    }`,
                );
            });
        ...
        const findingsChecklist: string[] = [
            "Use this as the implementation checklist. Keep code unchanged for confirmed false positives and document the evidence.",
        ];
        sortedRuleEntries.forEach(([ruleId, data], index) => {
            ...
            const ruleLines = [
                `### ${index + 1}. ${ruleId} [${data.sample.severity}, ${promptLaneForFinding(data.sample)}]`,
                `Pattern: ${data.sample.title}`,
                data.sample.suggested_fix ? `Fix guidance: ${compact(data.sample.suggested_fix, 260)}` : "",
            ].filter(Boolean);
            Object.keys(ruleFiles)
                .sort((a, b) => a.localeCompare(b))
                .forEach((file) => {
                    const matches = ruleFiles[file].sort((a, b) => (a.line_start ?? 0) - (b.line_start ?? 0));
                    ruleLines.push(`- ${displayPath(file)}`);
                    matches.forEach((finding) => {
                        ruleLines.push(`  - ${promptFindingLocation(finding)}: ${finding.title}`);
                        if (finding.description) ruleLines.push(`    Issue: ${compact(finding.description, 180)}`);
                        if (finding.why_it_matters) ruleLines.push(`    Impact: ${compact(finding.why_it_matters, 160)}`);
                    });
                });
        ...
        return buildPrompt([
            ...
            { type: "findings", title: "Findings By Rule And File", content: findingsChecklist.join("\n\n") },
    ...
    const buildProjectPrompt = () => {
        ...
        const highestSeverity = topActions
            .map((action) => action.max_severity)
            .sort((a, b) => (severityRank[b] ?? 0) - (severityRank[a] ?? 0))[0];
        const topActionFingerprints = new Set(topActions.flatMap((action) => action.finding_fingerprints));
        const topActionFindings = (report.findings ?? []).filter((finding) => topActionFingerprints.has(finding.fingerprint));
        const projectPromptPriority = topActionFindings.length > 0
            ? promptPriorityFromFindings(topActionFindings)
            : promptPriorityFromSeverity(highestSeverity);
        const priorityQueue = topActions.map((action, index) => {
            return `${index + 1}. [${action.max_severity}] ${action.title}\n   ${action.rule_id}: ${action.files.length} files, ${action.finding_fingerprints.length} findings${
                action.suggested_fix ? `\n   Fix: ${compact(action.suggested_fix, 180)}` : ""
            }`;
        });
        const actionFileIndex = topActions.flatMap((action, actionIndex) => {
            const files = action.files.length > 0 ? action.files : [];
            if (files.length === 0) {
                return [`${actionIndex + 1}. ${action.rule_id}: no files attached to this action.`];
            }
            return [
                `${actionIndex + 1}. ${action.rule_id}: ${files.length} file(s)`,
        ...
        return buildPrompt([
            createPromptContext({
                intent: topActionFindings.length > 0 ? promptIntentFromFindings(topActionFindings) : "fix",
                scope: "project",
                priority: projectPromptPriority,
                architecture: topActionFindings.length > 0
                    ? inferPromptArchitectureFromFindings(topActionFindings, detectedArchitecture)
                    : cleanFallbackArchitecture(detectedArchitecture),
                summary: [
                    `Prioritized issues: ${topActions.length}`,
            ...
            }),
            {
                type: "instruction",
                content: [
                    "Work through the priority queue in order.",
                    "Apply consistent fixes across files that share the same rule.",
                    promptPriorityInstruction[projectPromptPriority],
                ],
            },
            { type: "findings", title: "Priority Queue", content: priorityQueue },
            { type: "findings", title: "Full Action File Index", content: actionFileIndex },
            createStandardPromptConstraints([
                "Preserve public APIs unless the finding requires a breaking change.",
                "Consider ripple effects across related components and backend/frontend contracts.",
            ]),
            { type: "verification", content: ["Run tests after each rule is fixed when the blast radius is broad.", "Re-scan to verify fixes."] },
            createStandardPromptOutput(["Short summary of completed project-level risk reduction"]),
        ]);
    };
