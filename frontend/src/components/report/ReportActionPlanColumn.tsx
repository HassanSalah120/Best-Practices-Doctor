  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className="flex w-full items-start justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 text-left transition-colors hover:bg-white/[0.06]"
      >
        <div>
          <div className="flex items-center gap-2 text-sm font-semibold text-white">
            {collapsed ? <ChevronRight className="h-4 w-4 text-white/45" /> : <ChevronDown className="h-4 w-4 text-white/45" />}
            {title}
            <Badge variant="outline" className="border-white/10 bg-slate-900/60 text-[10px] text-white/70">
              {items.length}
            </Badge>
          </div>
