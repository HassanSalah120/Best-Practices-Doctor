...
export function PRGatePanel({ jobId }: PRGatePanelProps) {
  ...
  const [preset, setPreset] = useState<"startup" | "balanced" | "strict">("balanced");

  const runGate = async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await ApiClient.runPRGate(jobId, preset);
      setResult(data);
    } catch (err) {
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {(["startup", "balanced", "strict"] as const).map((item) => (
              <button
                key={item}
                type="button"
