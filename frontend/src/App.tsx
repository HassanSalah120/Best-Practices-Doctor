...
function App() {
  ...
  const startScan = async (options: StartScanOptions) => {
    ...
    try {
      const selectedProfile = options.baseline_profile ?? activeProfile;
      ...
      const { job_id } = await ApiClient.startScan(options);
      setProjectContextOverrides(options.project_context_overrides);
      setJobId(job_id);
      setView("progress");
    } catch (err) {
