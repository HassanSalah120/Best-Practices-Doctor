...
function isIgnoredPath(relPosix: string): boolean {
  ...
  return (
    ...
    p.includes("/bower_components/") ||
    p.includes("/.git/") ||
    p.includes("/storage/") ||
    p.includes("/bootstrap/cache/")
  );
}
...
// A) Scan tools
server.tool("bpdoctor_health",
  "Check the Best Practices Doctor backend health and authentication status.", {}, async () => {
  try {
...
server.tool("bpdoctor_start_scan",
  "Start a BPD scan for a project path and store it as the active scan.",
  { path: z.string().min(1) },
  async ({ path: scanPath }) => {
...
server.tool("bpdoctor_get_scan",
  "Fetch scan status, score summary, and finding counts for a scan job.", { job_id: z.string().min(1) }, async ({ job_id }) => {
...
server.tool("bpdoctor_wait_scan",
  "Poll a scan job until it completes or the timeout is reached.",
  {
...
server.tool("bpdoctor_compare_baseline",
  "Compare the active scan against the saved baseline fingerprints.",
  {
...
server.tool("bpdoctor_save_baseline",
  "Save the active scan findings as a named baseline on the backend.",
  {
    profile: z.string().optional(),
  },
  async ({ profile }) => {
...
server.tool("bpdoctor_pr_gate",
  "Evaluate the active scan against PR gate thresholds.",
  {
...
server.tool("bpdoctor_set_active_scan",
  "Set the active scan job used by follow-up BPD MCP tools.", { job_id: z.string().min(1) }, async ({ job_id }) => {
  const st = await loadState();
...
server.tool("bpdoctor_get_active_scan",
  "Return the currently active scan job id.", {}, async () => {
  const st = await loadState();
  return { content: [{ type: "text", text: JSON.stringify({ active_job_id: st.active_job_id }) }] };
});

server.tool("bpdoctor_rescan_last_path",
  "Start a new scan for the last scanned project path.", {}, async () => {
  const st = await loadState();
...
server.tool("bpdoctor_list_files",
  "List files with findings from the active scan, optionally filtered.",
  {
    filter: z
      .object({
...
server.tool("bpdoctor_next_finding",
  "Return the next highest-priority finding from the active scan.",
  {
    filters: z
      .object({
...
server.tool("bpdoctor_get_finding",
  "Fetch one finding by fingerprint from the active scan.",
...
server.tool("bpdoctor_explain_finding",
  "Explain why a specific finding was flagged and why it matters.",
  { fingerprint: z.string().min(6) },
  async ({ fingerprint }) => {
    const id = await requireActiveScanId();
...
server.tool("bpdoctor_suggest_fix",
  "Return suggested remediation guidance for a specific finding.",
  { fingerprint: z.string().min(6) },
  async ({ fingerprint }) => {
    const id = await requireActiveScanId();
    const resp = await httpJson<any>(
...
server.tool("bpdoctor_group_fixes",
  "Group active scan findings by rule, file, or fix strategy.",
  {
...
server.tool("bpdoctor_set_status",
  "Set local MCP workflow status for a finding fingerprint.",
  {
    fingerprint: z.string().min(6),
...
server.tool("bpdoctor_set_baseline_from_active_scan",
  "Save the active scan fingerprints as the local MCP baseline.", {}, async () => {
  const st = await loadState();
  const active = await requireActiveScanId();
  const scan = await httpJson<ScanResponse>("GET", apiUrl(`/api/scan/${active}`));
  const findings = extractFindings(scan);
...
server.tool("bpdoctor_clear_baseline",
  "Clear the local MCP baseline.", {}, async () => {
  const st = await loadState();
  st.baseline = null;
  await saveState(st);
...
registerRemediationTools(server, { apiUrl, httpJson, requireActiveScanId });

// C) Context helpers (read-only filesystem)
server.tool("repo_snippet",
  "Read a bounded snippet from a file inside the configured workspace root.",
  {
    path: z.string().min(1),
...
server.tool("repo_search",
  "Search text in files inside the configured workspace root.",
  {
